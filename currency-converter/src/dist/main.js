...
(() => {
  ...
  var require_invokeMap = __commonJS({
    "node_modules/lodash/invokeMap.js"(exports, module) {
      ...
      var invokeMap2 = baseRest(function(collection, path, args) {
        var index = -1, isFunc2 = typeof path == "function", result = isArrayLike(collection) ? Array(collection.length) : [];
        baseEach(collection, function(value) {
          result[++index] = isFunc2 ? apply(path, value, args) : baseInvoke(value, path, args);
        });
        return result;
      });
  ...
  function verifyResponseBodyType(body) {
    if (typeof body != "string") {
      var error2 = new Error("Attempted to respond to fake XMLHttpRequest with " + body + ", which is not a string.");
      error2.name = "InvalidBodyException";
      throw error2;
    }
  }
  ...
  function Url(address, location, parser) {
    ...
    for (; i < instructions.length; i++) {
      ...
      if (parse3 !== parse3) {
        ...
        if (~(index = address.indexOf(parse3))) {
  ...
  var Pretender = (
    ...
    (function() {
      ...
      Pretender2.prototype.handleRequest = function(request) {
        ...
        if (handler) {
          ...
          try {
            ...
            this.erroredRequest(verb, path, request, error2);
      ...
      Pretender2.prototype.erroredRequest = function(verb, path, _request, error2) {
        error2.message = "Pretender intercepted " + verb + " " + path + " but encountered an error: " + error2.message;
        throw error2;
      };
    ...
    _createClass(DbCollection2, [{
      ...
      value: function remove2(target) {
        ...
        if (typeof target === "undefined") {
          ...
          records.forEach(function(record2) {
            var index2 = _this3._records.indexOf(record2);
            _this3._records.splice(index2, 1);
          });
        } else if (_typeof(target) === "object") {
  ...
  var HasMany = /* @__PURE__ */ (function(_Association) {
    ...
    _createClass(HasMany2, [{
      ...
      value: function addMethodsToModelClass(ModelClass, key) {
        ...
        modelPrototype["new".concat(capitalize2(camelize2(this._container.inflector.singularize(association.name))))] = function() {
          var modelName, attrs;
          if (association.isPolymorphic) {
            modelName = arguments.length <= 0 ? void 0 : arguments[0];
            attrs = arguments.length <= 1 ? void 0 : arguments[1];
        ...
        modelPrototype["create".concat(capitalize2(camelize2(this._container.inflector.singularize(association.name))))] = function() {
          var modelName, attrs;
          if (association.isPolymorphic) {
            modelName = arguments.length <= 0 ? void 0 : arguments[0];
            attrs = arguments.length <= 1 ? void 0 : arguments[1];
      ...
      value: function _getAttrsForRequest(request, modelName) {
        ...
        if (json.data.relationships) {
          Object.keys(json.data.relationships).forEach(function(relationshipName) {
            ...
            var association = modelClass.associationFor(camelize2(relationshipName));
  ...
  var Util = class _Util {
    ...
    static loadCurrencies() {
      for (let obj of Object.entries(currenciesJson)) {
        _Util.currencies.push(new Currency(obj[0], obj[1]));
      }
      return _Util.currencies;
    }
    static async convertCurrency(from, to2, amount) {
      const result = await fetchForexRates(from);
      if (result instanceof Error) {
        return new Result(result.message + ": could not retrieve exchange rate data.");
      } else {
        const rate = result[0].rates[to2];
        const convertedAmount = amount * rate;
        _Util.result = new Result(convertedAmount);
        return _Util.result;
      }
    }
  };
  ...
  var fromAmount = document.getElementById("from_amount");
  var toAmount = document.getElementById("to_amount");
  var error = document.getElementById("error");
  var userLocale = navigator.languages && navigator.languages.length ? navigator.languages[0] : navigator.language;
  var currencyObjArray = Util.loadCurrencies();
  function generateOptions() {
    const currencyArray = currencyObjArray.map((element) => [element.getCode(), element.getName()]);
    return Object.values(currencyArray).map(([code, name]) => `<option value="${code}">${code} - ${name}</option>`).join("");
  }
  ...
  async function handleInput(e) {
    ...
    if (amount == "") {
      ...
      const result = await Util.convertCurrency(from || "", to2 || "", amount);
      const value = result.getValue();
      toAmount.textContent = formatCurrency(value, to2);
